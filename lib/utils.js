...
calculateScore = function calculateScore(questions){
    ...
    .reduce(function (data, question) {
        ...
        if(answer.skill){
            data.answered[skill2category[answer.skill]]++;
...
genQuizQuestionSet =  function genInviteQuestionSet(name) {
    ...
    //FIXME:
    answers = _.last(answers, Math.min(0, Math.floor(answers.length * 0.5) * 2) );
    var result = [];
    for(var i = 0; i < answers.length; i+=2) {
        result.push({ text : questionText, answers : [ answers[i], answers[i+1] ]});
    }

    var type3questions = _.chain(qdata.type3).shuffle().last(10).map(function(question){
